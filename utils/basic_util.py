...
def visualize(feat, target_labels, sensitive_labels, epoch):
    plt.ion()
    ...
    for i in range(m):
        for j in range(n):
            ...
            plt.plot(feat[index, 0], feat[index, 1], '.', c=c[j], marker=marker[i])
    ...
    # plt.xlim(left=np.min(feat[:, 0]), right=np.max(feat[:, 0]))
    # plt.ylim(bottom=np.min(feat[:, 1]), top=np.max(feat[:, 1]))
